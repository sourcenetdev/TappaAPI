(function (factory) {
    ...
    $.extend($.summernote.plugins, {
        ...
        'snvariable': function (context) {
            var ui = $.summernote.ui;
            var options = context.options.snvariable;
            var context_options = context.options;
            var lang = context_options.langInfo;
            var defaultOptions = {
                label: lang.snvariable.Variable,
                tooltip: lang.snvariable.Insert_variable
            };
            for (var propertyName in defaultOptions) {
                if (options.hasOwnProperty(propertyName) === false) {
                    options[propertyName] = defaultOptions[propertyName];
            ...
            context.memo('button.snvariable', function () {
                var button = ui.buttonGroup([
                    ui.button({
                        className: 'dropdown-toggle',
                        contents: '<span class="snvariable"> ' + options.label + '</span><span class="note-icon-caret"></span>',
                        tooltip: options.tooltip,
                        data: {
                            toggle: 'dropdown'
                        }
                    ...
                    ui.dropdown({
                        ...
                        click: function (event) {
                            ...
                            var node = document.createElement('span');
                            node.innerHTML = value;
                            context.invoke('editor.insertText', '{{{variable:' + value + '}}}');
                        }
                    })
                ]);
                return button.render();
